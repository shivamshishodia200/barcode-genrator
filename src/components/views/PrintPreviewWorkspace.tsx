...
export const PrintPreviewWorkspace: React.FC<PrintPreviewWorkspaceProps> = ({
  ...
  // Auto-fit on mount and when resizing
  useEffect(() => {
    const timer = setTimeout(() => {
      handleFitPage();
    }, 60);
    window.addEventListener('resize', handleFitPage);
    return () => {
      clearTimeout(timer);
      window.removeEventListener('resize', handleFitPage);
    };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#9fbddb] text-slate-800 select-none font-sans overflow-hidden">
      ...
      <div className="h-9 bg-gradient-to-b from-[#f8faff] to-[#e4edf7] border-b border-[#a4bed8] flex items-center justify-between px-2 shrink-0 shadow-2xs text-xs">
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-1.5 bg-white hover:bg-slate-50 border border-[#b4c8dc] rounded text-slate-700 shadow-2xs cursor-pointer font-bold text-[10px]"
            title="Actual Physical Size (100%)"
          >
            1:1
          </button>
      ...
      >
        {viewMode === 'single' ? (
          // Single Page Physical Sheet View (Matching Screenshot 4)
          currentPage && (
            <div
